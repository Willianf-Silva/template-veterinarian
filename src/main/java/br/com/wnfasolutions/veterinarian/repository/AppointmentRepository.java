...
public interface AppointmentRepository extends JpaRepository<AppointmentDO, Long> {
	...
	@Query(value = "SELECT a.* FROM tb_appointment AS a WHERE a.status = ?1 ORDER BY a.date, a.veterinarian_id", 
	...
	@Query(value = "SELECT a.* FROM tb_appointment AS a WHERE a.date = ?1 ORDER BY a.veterinarian_id, a.client_id", 
			countQuery = "SELECT count(*) FROM appointment AS a WHERE a.date = ?1", 
			nativeQuery = true)
	Page<AppointmentDO> findByDate(String date, Pageable pageable);
	
	@Query(value = "SELECT a.* FROM tb_appointment AS a WHERE a.status = ?1 AND a.date = ?2 ORDER BY a.veterinarian_id, a.client_id", 
